...
public:
    ...
    struct StageTimingMs {
        ...
        // GPU timestamp query results for the combined postprocess + tooltip command list.
        // These are pure GPU execution intervals measured on the D3D12 command queue.
        double gpu_postprocess = 0.0;
        double gpu_tooltip = 0.0;
        double gpu_postprocess_tooltip_total = 0.0;
...
private:
    ...
    struct PendingGpuFrame {
        ...
        UINT gpu_timestamp_query_base = 0;
    };
    ...
    void create_gpu_timestamp_resources();
    void write_gpu_timestamp(ID3D12GraphicsCommandList* command_list, UINT query_index);
    void resolve_gpu_timestamps(ID3D12GraphicsCommandList* command_list, UINT query_base, UINT query_count);
    void fill_gpu_timestamp_timing(PendingGpuFrame& pending, StageTimingMs& timing);
...
private:
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> gpu_timestamp_readback_buffer_;
    UINT64 gpu_timestamp_frequency_ = 0;
    UINT gpu_timestamp_query_count_ = 3;
