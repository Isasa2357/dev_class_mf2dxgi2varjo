...
UINT64 D3D12Core::close_execute_and_signal()
{
    ...
    if (!this->command_queue_ || !this->fence_) {
        throw std::runtime_error("D3D12Core::close_execute_and_signal: queue/fence is null");
    }
...
bool D3D12Core::is_fence_complete(UINT64 fence_value) const
{
    if (!this->fence_) {
        throw std::runtime_error("D3D12Core::is_fence_complete: fence is null");
    }

    return this->fence_->GetCompletedValue() >= fence_value;
}

void D3D12Core::wait_for_fence(UINT64 fence_value)
{
    if (!this->fence_) {
        throw std::runtime_error("D3D12Core::wait_for_fence: fence is null");
    }
    if (!this->fence_event_) {
        throw std::runtime_error("D3D12Core::wait_for_fence: fence_event is null");
    }

    if (this->fence_->GetCompletedValue() < fence_value) {
        HRESULT hr = this->fence_->SetEventOnCompletion(
            fence_value,
            this->fence_event_
        );
...
{
    const UINT64 fence_value = this->close_execute_and_signal();
    this->wait_for_fence(fence_value);
}
