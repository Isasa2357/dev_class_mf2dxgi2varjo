...
public:
    struct Config {
        ...
        // Stage-1 ring buffering: only the D3D11/D3D12 shared NV12 bridge is
        // replicated. The pipeline API remains sequential; each input frame uses
        // the next bridge slot in round-robin order.
    ...
    struct PipelineFrameResult {
        ...
        // Internal bridge ring-buffer slot used for this frame. Useful for
        // debugging frame/slot correspondence when ring buffering is expanded.
