...
public:
    ...
    // Async API step 1
    // -----------------------------------------------------------------
    // This is an API-level migration stage. submit_frame() still processes
    // the frame synchronously internally by calling process_frame(), then
    // stores the completed result in an internal queue. poll_result() returns
    // completed results in FIFO order.
    //
    // The purpose is to let callers migrate from:
    //   result = process_frame(frame);
    // to:
    //   submit_frame(frame);
    //   while (auto result = poll_result()) { ... }
    // without changing GPU scheduling yet. Later stages can make
    // submit_frame() actually non-blocking while keeping this public API.
    bool submit_frame(
        const D3D11VideoFrame& frame
    );
    ...
    // Step 1 has no in-flight asynchronous GPU work, so flush() is currently
    // a no-op placeholder. It exists so main code will not need to change
    // when true async execution is introduced.
    void flush();
...
private:
    ...
    // ORT session is still shared in this stage. Its D3D12 output buffers are
    // consumed immediately by the per-slot postprocessor before the next frame.
    // A later async stage can add ORT output slots and make submit_frame()
    // truly non-blocking while preserving the public submit/poll API.
