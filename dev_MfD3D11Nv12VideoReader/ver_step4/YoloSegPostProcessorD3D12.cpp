...
void YoloSegPostProcessorD3D12::record_external_outputs(
...
)
{
    if (!command_list) {
        throw std::runtime_error(
            "YoloSegPostProcessorD3D12::record_external_outputs: command_list is null"
        );
    }

    if (!external_output0_buffer || !external_output1_buffer) {
        throw std::runtime_error(
            "YoloSegPostProcessorD3D12::record_external_outputs: external output buffer is null"
        );
    }
