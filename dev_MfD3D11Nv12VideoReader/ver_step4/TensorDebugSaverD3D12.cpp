...
namespace {
    ...
    uint8_t ClampU8FromFloat255(float v)
    {
        v = std::clamp(v, 0.0f, 255.0f);
        return static_cast<uint8_t>(v + 0.5f);
    }

    void Nv12PixelToRgbLimitedBt601(
        uint8_t y_u8,
        uint8_t u_u8,
        uint8_t v_u8,
        uint8_t& r,
    ...
    {
        const float y = static_cast<float>(y_u8);
        const float u = static_cast<float>(u_u8) - 128.0f;
        const float v = static_cast<float>(v_u8) - 128.0f;

        // BT.601 limited range. Same intent as the current preprocess shader.
        const float yy = 1.164383f * (y - 16.0f);

        const float rf = yy + 1.596027f * v;
        const float gf = yy - 0.391762f * u - 0.812968f * v;
        const float bf = yy + 2.017232f * u;

        r = ClampU8FromFloat255(rf);
        g = ClampU8FromFloat255(gf);
        b = ClampU8FromFloat255(bf);
    }

    std::vector<uint8_t> ReadbackD3D11Nv12TextureAsRgb(
    ...
    )
    {
        if (!d3d11_core.device() || !d3d11_core.context()) {
            throw std::runtime_error("ReadbackD3D11Nv12TextureAsRgb: D3D11Core is not initialized");
        }
        if (!nv12_texture) {
            throw std::runtime_error("ReadbackD3D11Nv12TextureAsRgb: nv12_texture is null");
        }
        if (width == 0 || height == 0) {
            throw std::runtime_error("ReadbackD3D11Nv12TextureAsRgb: invalid image size");
        }
        ...
        if (src_desc.Format != DXGI_FORMAT_NV12) {
            throw std::runtime_error("ReadbackD3D11Nv12TextureAsRgb: source texture is not DXGI_FORMAT_NV12");
        }
        if (width > src_desc.Width || height > src_desc.Height) {
            throw std::runtime_error("ReadbackD3D11Nv12TextureAsRgb: requested size is larger than source texture");
        }
        ...
        win_util::ThrowIfFailed(hr, "Create NV12 staging texture failed");
        ...
        win_util::ThrowIfFailed(hr, "Map NV12 staging texture failed");
        ...
        const uint8_t* uv_plane =
            y_plane + static_cast<size_t>(mapped.RowPitch) * static_cast<size_t>(height);

        for (UINT y = 0; y < height; ++y) {
            const uint8_t* y_row =
                y_plane + static_cast<size_t>(mapped.RowPitch) * y;

            const uint8_t* uv_row =
                uv_plane + static_cast<size_t>(mapped.RowPitch) * (y / 2);

            for (UINT x = 0; x < width; ++x) {
                const uint8_t yv = y_row[x];
                const UINT uv_x = (x / 2) * 2;
                const uint8_t u = uv_row[uv_x + 0];
                const uint8_t v = uv_row[uv_x + 1];

                uint8_t r = 0;
                uint8_t g = 0;
                uint8_t b = 0;
                Nv12PixelToRgbLimitedBt601(yv, u, v, r, g, b);
    ...
    float InputToOriginalCoord(
        float input_coord,
        float letterbox_pad,
        float letterbox_scale,
        float max_original_coord
    )
    {
        ...
        float v = (input_coord - letterbox_pad) / letterbox_scale;
        return std::clamp(v, 0.0f, max_original_coord);
    }

    void DrawOriginalTipMarker(
        std::vector<uint8_t>& rgb,
        UINT width,
        UINT height,
        int x,
...
{
    if (!output_path) {
        throw std::runtime_error("SaveD3D11Nv12TextureWithOriginalToolTipsAsBmp: output_path is null");
    }
    if (input_width <= 0.0f || input_height <= 0.0f || mask_width <= 0.0f || mask_height <= 0.0f) {
        throw std::runtime_error("SaveD3D11Nv12TextureWithOriginalToolTipsAsBmp: invalid coordinate size");
    }

    std::vector<uint8_t> rgb = ReadbackD3D11Nv12TextureAsRgb(
        d3d11_core,
        nv12_texture,
        source_subresource_index,
        width,
    ...
    for (const auto& tip : tip_results) {
        ...
        if (draw_candidates) {
            const float c1_input_x = tip.candidate1_x_mask * input_width / mask_width;
            const float c1_input_y = tip.candidate1_y_mask * input_height / mask_height;
            const float c2_input_x = tip.candidate2_x_mask * input_width / mask_width;
            const float c2_input_y = tip.candidate2_y_mask * input_height / mask_height;
            ...
            DrawLineRgb(rgb, width, height, c1_x, c1_y, c2_x, c2_y, 255, 255, 255);
            DrawCrossRgb(rgb, width, height, c1_x, c1_y, 255, 255, 0, 10);
            DrawCrossRgb(rgb, width, height, c2_x, c2_y, 0, 255, 255, 10);
        }
        ...
        if (draw_axis) {
            ...
            DrawLineRgb(rgb, width, height, ax0, ay0, ax1, ay1, cr, cg, cb);
        }

        DrawOriginalTipMarker(rgb, width, height, tip_x, tip_y, cr, cg, cb);
    }
...
{
    if (!output_path) {
        throw std::runtime_error("SaveD3D11Nv12TextureWithOriginalMasksAndToolTipsAsBmp: output_path is null");
    }
    if (input_width <= 0.0f || input_height <= 0.0f || mask_width <= 0.0f || mask_height <= 0.0f) {
        throw std::runtime_error("SaveD3D11Nv12TextureWithOriginalMasksAndToolTipsAsBmp: invalid coordinate size");
    }
    if (letterbox_scale <= 0.0f) {
        throw std::runtime_error("SaveD3D11Nv12TextureWithOriginalMasksAndToolTipsAsBmp: invalid letterbox_scale");
    }
    ...
    std::cout
        ...
        << " subresource=" << source_subresource_index
        << "\n";
    ...
    // ------------------------------------------------------------
    // Mask overlay only. No bbox is drawn here.
    //
    // mask_results[].mask is 160x160 postprocess mask in YOLO input space.
    // To overlay on the original image, map original pixel -> YOLO input -> mask.
    // For speed in debug mode, iterate only the original-space bbox range.
    // ------------------------------------------------------------
    for (const auto& result : mask_results) {
        ...
        const size_t result_mask_pixels =
            static_cast<size_t>(result.mask_width) *
            static_cast<size_t>(result.mask_height);

        if (result.mask.size() < result_mask_pixels) {
            continue;
        }
        ...
        for (int y = y1; y <= y2; ++y) {
            ...
            if (input_y < 0.0f || input_y >= input_height) {
                continue;
            }

            int my = static_cast<int>(
                (input_y + 0.5f) * static_cast<float>(result.mask_height) / input_height
            );
            ...
            for (int x = x1; x <= x2; ++x) {
                ...
                if (input_x < 0.0f || input_x >= input_width) {
                    continue;
                }

                int mx = static_cast<int>(
                    (input_x + 0.5f) * static_cast<float>(result.mask_width) / input_width
                );
                ...
                const float mask_value = result.mask[mask_idx];
    ...
    // Draw this after masks so that markers remain visible.
    // ------------------------------------------------------------
    for (const auto& tip : tip_results) {
        if (tip.valid == 0) {
            continue;
