...
public:
    ...
    // Step 3: ORT output buffer slots.
    // Slot 0 is used by the legacy getters. For multiple in-flight frames,
    // call set_output_slot_count() and run_d3d12_input_and_upload_outputs_to_slot().
    void set_output_slot_count(size_t slot_count);
    size_t output_slot_count() const;

    std::vector<OutputTensor> run_d3d12_input_and_upload_outputs_to_slot(
    ...
    size_t gpu_output_count(size_t output_slot_index) const;
    ID3D12Resource* output_buffer(size_t output_slot_index, size_t output_index) const;
    D3D12_RESOURCE_STATES& output_state_ref(size_t output_slot_index, size_t output_index);
    ID3D12Resource* output0_buffer(size_t output_slot_index) const;
    ID3D12Resource* output1_buffer(size_t output_slot_index) const;
    D3D12_RESOURCE_STATES& output0_state_ref(size_t output_slot_index);
    D3D12_RESOURCE_STATES& output1_state_ref(size_t output_slot_index);
...
private:
    ...
    struct GpuOutputSlot {
        std::vector<GpuOutputBuffer> outputs;
    };

    std::vector<GpuOutputSlot> gpu_output_slots_;

    // Legacy compatibility: getters without a slot index read/write this slot.
    size_t active_output_slot_index_ = 0;
