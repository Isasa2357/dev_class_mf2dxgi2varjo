...
public:
    ...
    // Async API step 4
    // -----------------------------------------------------------------
    // submit_frame() records/submits final postprocess+tooltip GPU work using
    // a per-processing-slot command allocator/list. Multiple frames can now
    // remain in-flight as long as free bridge/processing slots exist.
    // poll_result() retires completed frames in order and performs the small
    // tooltip readback only after the associated fence has completed.
    bool submit_frame(
        const D3D11VideoFrame& frame
    );
...
private:
    struct ProcessingSlot {
        ...
        // Step 4: per-slot command objects for postprocess + tooltip GPU work.
        // These allow multiple submitted command lists to be in-flight without
        // resetting a command allocator still used by the GPU.
    ...
    bool retire_pending_frames(bool wait_for_oldest);
    bool retire_one_pending_frame(bool wait_for_completion);

    bool is_bridge_slot_pending(UINT bridge_slot_index) const;
    bool is_processing_slot_pending(UINT processing_slot_index) const;
    ...
    void create_processing_slot_command_objects(ProcessingSlot& slot) const;
    ID3D12GraphicsCommandList* reset_processing_slot_command_list(ProcessingSlot& slot);
    UINT64 close_execute_processing_slot_command_list(ProcessingSlot& slot);
...
private:
    ...
    std::deque<PendingGpuFrame> pending_gpu_frames_;
